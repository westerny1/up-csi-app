<script lang="ts">
	...
	const sections = [
		{
			title: 'Section 1',
			subItems: [
				'Section 1 sub item 1',
				'Section 1 sub item 2', 
				'Section 1 sub item 3',
				'Section 1 sub item 4'
			]
		},
		{
			title: 'Section 2',
	...
	function toggleSection(index:number) {
		if (sections[index]!.subItems.length === 0) return;
		openSection = openSection === index ? null : index;
	}
</script>
			...
			{#if section.subItems.length > 0 && openSection === index}
				<div class="ml-6 space-y-2 mt-2">
					{#each section.subItems as subItem (subItem)}
						<a 
							href="#" 
							class="block rounded px-4 py-3 text-lg text-gray-300 hover:bg-[#2C2C2F] hover:text-white transition-colors duration-200"
						>
							{subItem}
						</a>
					{/each}
